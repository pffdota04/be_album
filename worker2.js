  ...
  try {
    ...
    uploadWatch.on("change", (change) => {
      if (change.operationType + "" === "insert") {
        if (change.fullDocument.worker === 2) {
          count++;
          console.log(
            count + ": Worker 2 --START: " + change.fullDocument.name
          );
...
const handleUpload = async (id) => {
  ...
  try {
    ...
    console.log(`Worker 2 COMPLETE:  ${endTime + " vs " + startTime}ms for ` + img.name);
    return true;
  } catch (error) {
    console.log(`Worker 2 ERROR: ` + img.name);
    img.status = "error";
    img.save();
    fs.unlinkSync("imgs/" + id + ".png");
    return false;
